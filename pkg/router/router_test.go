...
func TestBuildGetUpdateAndDeleteRoutes(t *testing.T) {
	...
	store.InfoStore.RemoveRoute("arithmetic", "Add", "/arithmetic/Add", "GET")
	data, ok = GetRoute("GET", "/arithmetic/Add")
	test.Assert(t, !ok)
	store.InfoStore.AddRoute("arithmetic", "Add", "/arithmetic/Add", "GET")
	data, ok = GetRoute("GET", "/arithmetic/Add")
	test.Assert(t, ok)
	test.Assert(t, data.ServiceName == "arithmetic" && data.MethodName == "Add")
	store.InfoStore.ModifyRoute("arithmetic", "Add", "/arithmetic/Add", "GET", "/test", "GET")
	data, ok = GetRoute("GET", "/arithmetic/Add")
	test.Assert(t, !ok)
